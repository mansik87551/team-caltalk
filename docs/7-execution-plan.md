...
- [ ] FE-02 API 클라이언트(axios·JWT 인터셉터)
- [ ] FE-03 전역 상태 스토어
- [ ] FE-04 인증 화면 + 토큰 흐름
- [ ] FE-05 시간대 변환 유틸(date-fns-tz)
- [ ] FE-06 통합 화면 레이아웃(캘린더+채팅)
- [ ] FE-07 캘린더 뷰(FullCalendar)
- [ ] FE-08 일정 등록/수정 모달(팀장 한정·충돌 경고)
- [ ] FE-09 채팅 패널 + Daily Chat Log
...
- **규모**: M
- **완료 조건 (DoD)**:
  - [ ] `GET /health` 200 + DB 상태 반영(실패 시 503)
  - [ ] config에서만 env 로딩, 필수 키 누락 시 부팅 실패
  - [ ] 미들웨어 체인(CORS 화이트리스트·json·로깅·에러) 조립, 미처리 에러 표준 포맷 직렬화
  - [ ] pino가 password/token redact, Vitest 스위트 실행 통과
...
- **규모**: M
- **완료 조건 (DoD)**:
  - [ ] 7테이블 생성, timestamptz UTC, PK uuid
  - [ ] CHECK(start<end / role / status), UNIQUE(email, (user_id,team_id))
  - [ ] ERD 4장 인덱스 생성
  - [ ] FK ON DELETE 정책 적용, migrate up/down 무오류
...
- **규모**: M
- **완료 조건 (DoD)**:
  - [ ] `detectConflict` AC-01 true, 경계 AC-02 false(`<` not `≤`)
  - [ ] 자기 제외 AC-03, 종일 반열린 구간 정규화
  - [ ] `validateSchedule` AC-04 거부
  - [ ] `canTransition` AC-07/08, `can(role,action)` AC-05
  - [ ] 테스트명에 BR/AC ID 포함, 전부 통과
...
- **규모**: M
- **완료 조건 (DoD)**:
  - [ ] `POST /api/auth/signup` 201, bcrypt 해시, 중복 409
  - [ ] `POST /api/auth/login` 200 + JWT(만료 TTL)
  - [ ] 불일치/미존재 401, 검증 실패 400
  - [ ] 응답·로그에 해시·평문 미노출
...
- **규모**: M
- **완료 조건 (DoD)**:
  - [ ] 무토큰·잘못된 토큰 401(AC-06)
  - [ ] 비멤버 403, teamId+Membership 타팀 격리(BR-02)
  - [ ] 팀원 일정 CUD 403, Service 진입 전 차단(BR-03/AC-05)
  - [ ] validate 스키마 위반 400, 권한 분기 한 곳 집중
...
- **규모**: M
- **완료 조건 (DoD)**:
  - [ ] `POST /api/teams` 201, 생성자 team_leader 자동 부여(OI-2)
  - [ ] 가입 시 team_member 생성, UNIQUE 위반 409(BR-10)
  - [ ] `GET .../members` 멤버 한정 조회(BR-02)
  - [ ] 다중 팀 소속·각 팀 단일 역할 유지
...
- **규모**: L
- **완료 조건 (DoD)**:
  - [ ] `GET .../schedules?from&to` 멤버 조회(인덱스 활용)
  - [ ] `POST` 팀장 한정(AC-05), 유효성 400(AC-04), conflict 호출
  - [ ] 충돌 시에도 저장 201 + `conflicts[]` 경고(BR-08), 자기 제외(AC-03)
  - [ ] `PUT`/`DELETE` 팀장 한정, DB CHECK+앱 검증 이중
  - [ ] `ScheduleCreated/Updated/ConflictDetected` 이벤트 발행
...
- **규모**: M
- **완료 조건 (DoD)**:
  - [ ] WS 핸드셰이크 JWT 검증, 무효 토큰 거부
  - [ ] 멤버인 `team:<id>` 룸만 join, 비멤버 거부(BR-02)
  - [ ] 룸 단위 브로드캐스트, 도메인:행위 채널 네이밍
  - [ ] 재연결 동작, 단일 인스턴스 룸(Redis 미도입)
...
- **규모**: M
- **완료 조건 (DoD)**:
  - [ ] 작성 시 target_date 자동 부여(도메인 4.1)
  - [ ] `GET .../chat?date=` Daily Chat Log 조회(인덱스 활용)
  - [ ] 작성 시 팀 룸 chat:message 브로드캐스트(NFR-03)
  - [ ] created_at(불변 UTC)·target_date 분리, 비멤버 403
...
- **규모**: L
- **완료 조건 (DoD)**:
  - [ ] `POST .../schedule-change-requests` → requested, origin_message_id NOT NULL 연결
  - [ ] `GET ...?status=requested` 목록(인덱스)
  - [ ] `PATCH .../:id` 팀장 applied/rejected, processed_* 기록, Applied 시 Schedule 반영(AC-07)
  - [ ] 종결 재전이 409, 팀원 처리 403(AC-08); SCR 이벤트 발행
...
- **규모**: M
- **완료 조건 (DoD)**:
  - [ ] ConflictDetected→팀장, Updated→팀원, Requested→팀장, Applied/Rejected→요청자
  - [ ] Notification 영속 저장 + WS 푸시
  - [ ] `GET /api/notifications?unread=true`(인덱스)
  - [ ] 이벤트 디스패치 Service 일원화
...
- **규모**: M
- **완료 조건 (DoD)**:
  - [ ] 무토큰·비멤버 차단(AC-06), 팀원 CUD 403(AC-05)
  - [ ] 충돌 등록 201+경고(AC-01/02), 유효성 400(AC-04)
  - [ ] Requested→Applied + 종결 재전이 거부(AC-07/08)
  - [ ] 테스트명 BR/AC ID 포함, CI 게이트 통과
...
- **규모**: S
- **완료 조건 (DoD)**:
  - [ ] `npm run dev` HMR로 빈 셸 렌더
  - [ ] 라우트(`/login`, `/`) + 미인증 리다이렉트 가드 자리
  - [ ] QueryClientProvider + Tailwind 적용
  - [ ] ESLint+Prettier 통과, tsconfig strict
...
- **규모**: S
- **완료 조건 (DoD)**:
  - [ ] 로그인 시 token·user·role 저장 + 새로고침 복원(localStorage)
  - [ ] selectedDate 전역 구독으로 캘린더↔채팅 동기화
  - [ ] currentTeamId·역할 노출(권한 UI 기준)
  - [ ] logout 시 전체 초기화
...
- **규모**: M
- **완료 조건 (DoD)**:
  - [ ] 회원가입 성공 시 자동 로그인·토큰 저장·진입
  - [ ] 로그인 성공 시 TeamWorkspacePage 이동(SC-01)
  - [ ] 미인증 보호 라우트 접근 차단(BR-01/AC-06)
  - [ ] zod 검증·서버 에러 폼 표시
...
- **규모**: S
- **완료 조건 (DoD)**:
  - [ ] UTC→KST 정확 변환(05:00Z→14:00)
  - [ ] 로컬 입력→UTC ISO 변환 전송
  - [ ] selectedDate→targetDate(YYYY-MM-DD) 로컬 산출
  - [ ] 변환 로직이 이 모듈에만 존재
...
- **규모**: M
- **완료 조건 (DoD)**:
  - [ ] 한 화면에 캘린더+채팅 동시 배치(슬롯)
  - [ ] 팀 전환 시 currentTeamId 갱신·하위 리렌더
  - [ ] ≥1024px 2열, 좁은 폭 세로 스택(NFR-08)
  - [ ] 헤더에 사용자·역할·로그아웃
...
- **규모**: M
- **완료 조건 (DoD)**:
  - [ ] 월/일(Must)·주(Should) 뷰 KST 표시
  - [ ] 날짜 클릭 시 selectedDate 갱신→채팅 동기화(UC-06)
  - [ ] 팀 전환·변경 시 Query 무효화 자동 갱신
  - [ ] 종일 일정 종일 영역 렌더
...
- **규모**: L
- **완료 조건 (DoD)**:
  - [ ] 팀원에게 추가/수정/삭제 UI 미노출(BR-03)
  - [ ] start==end / end<start 클라이언트 거부(AC-04/EX-03)
  - [ ] 저장 성공 시 모달 닫힘·캘린더 무효화 반영(SC-02)
  - [ ] 서버 충돌 반환 시 저장 유지+경고 표시(BR-08/SC-03)
  - [ ] 로컬→UTC 변환(FE-05) 후 전송
...
- **규모**: M
- **완료 조건 (DoD)**:
  - [ ] 날짜 선택 시 해당 targetDate Daily Chat Log 동기화(SC-04)
  - [ ] 전송 시 selectedDate가 targetDate로 부여(도메인 4.1)
  - [ ] createdAt KST 표시, targetDate≠createdAt 구분
  - [ ] 며칠 뒤 같은 날짜 재조회 시 대화 보존(BR-05)
...
- **규모**: M
- **완료 조건 (DoD)**:
  - [ ] 로그인·팀 진입 시 JWT 핸드셰이크·룸 join
  - [ ] 타 사용자 메시지 1초 내 반영(NFR-03)
  - [ ] 일정 이벤트 수신 시 캘린더 자동 갱신
  - [ ] 팀 전환 룸 leave/join, 끊김 시 재연결
...
- **규모**: L
- **완료 조건 (DoD)**:
  - [ ] 팀원 [변경 요청]만, 처리 버튼 미노출(BR-04)
  - [ ] 생성 시 채팅 메시지 기록·originMessageId 연결(SC-05)
  - [ ] 팀장 Requested 목록·원본 채팅 링크 확인(SC-05)
  - [ ] Applied 시 팀 반영, Rejected 시 불변·사유 표시(SC-05/06/AC-07)
  - [ ] 종결 요청 처리 버튼 비활성(AC-08/EX-05)
...
- **규모**: S
- **완료 조건 (DoD)**:
  - [ ] 충돌 감지 시 경고 토스트(SC-03)
  - [ ] 요청 생성·Applied·Rejected 통지(SC-05/06)
  - [ ] 중복 없이 누적·해제, 세션 동안 확인(OI-5)
...
- **규모**: S
- **완료 조건 (DoD)**:
  - [ ] 팀원 통제 UI 일관 숨김(SC-01/AC-05 UX)
  - [ ] 팀장 모든 통제 액션 노출
  - [ ] 권한 분기 RoleGate 한 곳 집중(중복 없음)
...
- **규모**: M
- **완료 조건 (DoD)**:
  - [ ] <1024px 캘린더·채팅·모달 붕괴 없음(NFR-08)
  - [ ] 4개 브라우저 최신 2버전 동작(NFR-07)
  - [ ] SC-01~06 해피패스 end-to-end 수동 통과
  - [ ] React.lazy 번들 분리
...
- [ ] DB 6개, BE 12개, FE 14개 Task의 DoD 체크박스 전부 충족
- [ ] 핵심 수용 기준 AC-01~AC-08 자동/수동 검증 통과
- [ ] 사용자 시나리오 SC-01~SC-06 해피패스 end-to-end 동작
- [ ] 보안 Hard Rule(서버측 권한 강제·bcrypt 해시·파라미터라이즈드 쿼리·UTC 저장) 준수 확인
- [ ] CI 최소 게이트(lint + 단위테스트) 통과
